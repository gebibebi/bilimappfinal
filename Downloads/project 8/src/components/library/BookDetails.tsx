  ...
  const handleStartAnalysis = () => {
    ...
    navigate('/chat', { 
      state: { 
        bookId: book.id,
        action: 'analysis',
        title: `${t('Book Analysis')}: ${book.title}`,
      ...
      >
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              <h3 className="font-medium text-gray-900 mb-2">{t('Information')}</h3>
              <dl className="space-y-2">
                <div>
                  <dt className="text-sm text-gray-500">{t('Subject')}</dt>
                  <dd className="font-medium text-gray-900">{book.subject}</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">{t('Grade')}</dt>
                  <dd className="font-medium text-gray-900">{book.grade} {t('grade')}</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">{t('Author')}</dt>
                  <dd className="font-medium text-gray-900">{book.author}</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">{t('Publisher')}</dt>
                  <dd className="font-medium text-gray-900">{book.details?.publisher}</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">{t('Publication Year')}</dt>
                  <dd className="font-medium text-gray-900">{book.details?.year}</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">{t('Pages')}</dt>
                ...
                <div>
                  ...
                  <dd className="font-medium text-gray-900">
                    {book.details?.languages?.map(lang => 
                      lang === 'russian' ? t('Russian') : t('Kazakh')
                    ).join(' / ') || (book.language === 'russian' ? t('Russian') : t('Kazakh'))}
                  </dd>
                </div>
              </dl>
            </div>
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-2">{t('Contents')}</h3>
              {book.details?.contents ? (
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-gray-900">{t('Main Topics')}</h4>
                  <ul className="mt-2 space-y-1">
         ...
         {/* Video lessons section */}
          <div className="mt-6 border-t border-gray-200 pt-4">
            <h3 className="font-medium text-gray-900 mb-3">{t('Video Lessons by Topic')}</h3>
                    ...
                    {section.topics.slice(0, 2).map((topic, topicIdx) => (
                      <div key={topicIdx} className="pl-4 border-l-2 border-blue-200">
                        ...
                        <div className="mt-1 space-y-1">
                          ...
                          >
                            <PlayCircle size={16} />
                            <span>{t('How to solve')} {topic.toLowerCase()} {t('in 5 minutes')} (YouTube)</span>
                          </a>
                          <a 
                          ...
                          >
                            <PlayCircle size={16} />
                            <span>{t('Explanation of')} {topic.toLowerCase()} {t('from NIS teacher')}</span>
                          </a>
                        </div>
                      </div>
                    ))}
