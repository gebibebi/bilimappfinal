...
import { 
  ...
  LayoutGrid,
  AlertCircle,
  GraduationCap, // Иконка для категории "Поступление"
  Home
} from 'lucide-react';
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Check if coming from admissions page with a question
    else if (location.state?.action === 'admission_question') {
      const { question, sessionId } = location.state;
      
      // If we already have a session ID, just set it as current
      if (sessionId) {
        setCurrentSession(sessionId);
      }
    }
  }, [location, createSession, setCurrentSession]);
  ...
  const handleSendMessage = (content: string, audioBlob?: Blob, image?: string) => {
    ...
    // Создание вложений если есть аудио или изображение
    const attachments = [];
    if (audioBlob) {
      attachments.push({
        type: 'voice',
    ...
    setTimeout(() => {
      if (currentSession.category === 'analysis' && currentSession.bookId) {
        ...
        if (content.match(/^\d+$/) || content.toLowerCase().includes('тема')) {
          ...
          addMessage(`Я помогу тебе разобраться с этим материалом. Какой аспект этой темы тебе интересен больше всего? Если хочешь, можешь записать голосовой ответ.`, 'assistant');
        }
      } 
      ...
      // Если это категория поступления
...
Давай еще рассмотрим несколько примеров, чтобы закрепить материал.`, 'assistant');
  ...
  const categories = [
    ...
    { id: 'admissions', name: 'Поступление в вузы', icon: GraduationCap }, // Новая категория
  ];
...
<div className="chat-categories grid grid-cols-1 gap-2">
  ...
  const renderChatInterface = () => {
    ...
    return (
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <VoiceChatInput onMessageSend={handleSendMessage} />
      </div>
    );
  };
