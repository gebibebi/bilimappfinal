...
const [breathePhase, setBreathePhase] = useState<'inhale' | 'exhale'>('inhale');
  ...
  // Anti-stress detection timer - if student spends too much time on a question
useEffect(() => {
  if (!showResults && !showAntiStress) {
    // Setting a short timer for demo purposes (5 seconds)
    // In a real app, this might be 1-2 minutes
    const stressTimer = setTimeout(() => {
      setShowAntiStress(true);
    }, 30000); // 30 seconds in a real implementation
    
    return () => clearTimeout(stressTimer);
  }
}, [activeQuestionIndex, showResults, showAntiStress]);
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {/* Debug button - for demonstration purposes */}
<Button
  variant="outline"
...
<AnimatePresence>
  {showAntiStress && (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-blue-900 bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4 z-50"
    >
      <motion.div
      ...
      >
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Давай передохнем с помощью дыхательного упражнения
          </p>
        </div>
        ...
        <div className="flex justify-center mb-8">
          <motion.div
            animate={{
              scale: breathePhase === 'inhale' ? 1.3 : 1,
              opacity: breathePhase === 'inhale' ? 0.9 : 0.7,
            }}
            transition={{
              duration: 4,
              ease: 'easeInOut',
            ...
            onAnimationRepeat={() => {
              setBreathePhase(breathePhase === 'inhale' ? 'exhale' : 'inhale');
            }}
            className="w-40 h-40 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center"
          >
            <div className="text-white text-xl font-medium">
              {breathePhase === 'inhale' ? 'Вдох...' : 'Выдох...'}
            </div>
          </motion.div>
        </div>
        ...
        <div className="text-center">
          <p className="text-sm text-gray-500 mb-4">
            Следуйте за движением круга, делая глубокие вдохи и выдохи
          </p>
          <Button
            variant="primary"
            onClick={() => setShowAntiStress(false)}
          >
            Продолжить тест
          </Button>
        </div>
